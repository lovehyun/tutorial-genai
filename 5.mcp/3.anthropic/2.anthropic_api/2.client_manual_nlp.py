...
#
# 2.openai/1.agent_tool/2.client_manual_nlp.py 와 코드가 100% 동일하다 — 이유는 1.client_demo.py
# 와 같다: 아직 LLM이 등장하지 않아서 벤더가 다를 이유가 없다.
#
#   사용자 문장을 정규식으로 분석해 어떤 MCP 도구를 부를지 직접 고른다.
#   다음 단계(3.client_claude.py)에서 이 '선택'을 Claude 에게 맡긴다.
