# app.py — [3a] 오래 걸리는 업무는 서브에이전트에게 맡기고, 대화는 계속한다.
#
# ══ 이 폴더는 3단계의 세 변형(a/b/c) 중 하나다 — ../README.md 참고 ═══════
#   여기(a)는 모델이 한 턴에 도구를 딱 하나만 요청하도록 강제한다
#   (parallel_tool_calls=False). 그래서 승인 카드도 항상 도구 하나짜리만 뜬다 —
#   안전하지만, 온보딩 하나에 계정생성·email·vpn·메일까지 4번 연달아 승인해야 해서
#   피로가 쌓인다. `b.parallel_unsafe/`는 이 강제를 안 걸면 뭐가 위험해지는지 보여준다.
#
# ══ parallel_tool_calls=False 를 강제하는 법 ════════════════════
#   create_agent() 는 내부적으로 llm.bind_tools(tools, **model_settings) 를 호출하는데,
#   model_settings 는 top-level 파라미터로 노출돼 있지 않다 — 미리 llm.bind(parallel_tool_calls=False)
#   해서 넘겨도 create_agent 가 다시 bind_tools 를 걸 때 사라진다(직접 확인함).
#   대신 langchain 1.x 의 wrap_model_call 미들웨어로 request.model_settings 에 꽂아 넣는다 —
#   이게 공식적으로 노출된 유일한 통로다.
#
# ══ 2단계의 한계 ═══════════════════════════════════════════════
#   2단계는 승인 카드가 뜨면 대화가 그 자리에서 멈춘다. 승인할 때까지 아무것도 못 한다.
#   실제 업무는 그렇지 않다 — "정다은 온보딩 해줘" 를 시켜놓고 다른 일을 하다가,
#   승인이 필요한 순간에만 결정해 주면 된다.
#
# ══ 구조: 메인 에이전트 + 워커(서브에이전트) ════════════════════
#
#     사용자 ──대화──▶ 메인 에이전트   (조회 도구 + delegate_task/list_jobs)
#                          │ delegate_task("정다은 온보딩")
#                          ▼
...
def one_tool_call_per_turn(request, handler):
    """OpenAI 의 parallel tool calling 을 끈다.

    request.model_settings 는 실행 직전에 llm.bind_tools(tools, **model_settings) 로
    그대로 전달된다 — 여기서 꽂아두면 매 턴 확실히 적용된다.
    """
...
async def build():
    ...
    # [3a] 워커: MCP 도구 전부 + 위험한 도구에서 정지 + 한 턴에 도구 하나만
    worker_agent = create_agent(
        llm, tools,
        system_prompt=WORKER_SYSTEM,
        checkpointer=checkpointer,
        interrupt_before=["tools"],
        middleware=[one_tool_call_per_turn],
    )
