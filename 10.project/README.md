...
| `18.mcp_ops_hitl/` | **사내 IT 비서 — 다중 MCP + 사람 승인 + 서브에이전트** (4단계 빌드업) | G | ★★★ | MCP 3개, LangGraph interrupt, 체크포인터, 백그라운드 워커 |
...
### `18.mcp_ops_hitl/` — 사내 IT 비서: 다중 MCP + 사람 승인 + 서브에이전트 (★★★)
...
| `3.background_tasks/` | 5083·5086·5087 | + **서브에이전트 위임** — 백그라운드 실행, 작업 패널, 여러 건 동시 + 각자 승인 대기 (세 변형 `a`순차/`b`병렬-위험/`c`병렬-guarded) |
| `4.auto_approve/` | 5084·5085 | + **자동승인** — 한 번 승인한 기능은 안 묻기, 자동승인 목록·DB 현황 패널 (두 변형 `a`도구이름/`b`인자수준-guardrail) |

**학습 포인트**:
- **웹에는 `input()` 이 없다** — 멈춘 에이전트를 checkpointer 에 저장해 두고, 승인 버튼이 꺼내 재개시킨다
  (기다림을 프로세스가 아니라 저장소가 담당한다)
- MCP 서버는 4단계 내내 **한 줄도 안 바뀐다** — 안전장치는 서버가 아니라 서버를 쓰는 쪽이 건다
- 굵은 단위의 자동화는 위험도를 못 본다 — 3단계(승인 요청 단위)와 4단계(자동승인 등록 단위)가
  같은 문제를 다른 층에서 보여준다
- 메인/워커 에이전트 분리 + `asyncio.Event` 로 백그라운드 작업을 재우고 깨우기
- 승인 피로를 피하는 화이트리스트 설계, 거부를 '중단' 이 아니라 '대화' 로 만들기
- 자동승인으로 반복 승인을 없애되, **무엇을 위임했고 무엇이 바뀌었는지** 패널로 되보여 통제 유지
...
| | | | → E.17.mcp_marketplace | → G.18.mcp_ops_hitl |
