# OpenAI SDK - 11단계: Responses API 멀티턴 (previous_response_id 체인)
#
# 10단계 대비 새로 추가된 것:
#   대화를 '서버 측'에서 이어붙인다 — Responses API의 핵심 기능.
#
# 4.chat_multiturn.py 와의 차이:
...
#
# ../1.restapi/11.restapi_response_chain.py 의 SDK 버전이기도 하다 — 같은 일이지만 코드가 훨씬 짧다.
#
...
# chat.completions(4단계)에서는 messages 리스트에 사용자/모델 메시지를 매번 다 누적해야 했다.
