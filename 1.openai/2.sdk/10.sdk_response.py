# OpenAI SDK - 10단계: Responses API를 SDK로 호출 (기본)
# pip install openai python-dotenv
#
# 1~5단계는 chat.completions 방식이었다. 10단계부터는 Responses API — 완전히
# 다른 시리즈라 번호 대역을 일부러 띄웠다(6~9는 결번).
#
# ../1.restapi/10.restapi_response.py 는 raw HTTP(requests)로 /v1/responses 를 호출했다.
# 여기서는 같은 일을 openai SDK로 한다. SDK가 줄여주는 것:
...
#
# 멀티턴 체인(previous_response_id 활용)은 11.response_multiturn.py 에서 다룬다.
