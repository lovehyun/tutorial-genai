# OpenAI SDK - 5단계: 구버전 SDK (v0.x) — 참고용
# pip install openai==0.28
#
# 4단계까지는 REST API를 직접 호출했다. 5단계부터는 openai 라이브러리(SDK)를 쓴다.
# SDK는 HTTP 요청·헤더·JSON 파싱을 대신 해 주므로 코드가 짧아진다.
#
# 이 파일은 '구버전(v0.x)' 스타일. 지금은 v1.x가 표준이지만,
# 인터넷의 옛 예제를 읽을 때를 대비해 모습만 익혀 둔다 (신규 코드는 6단계 사용).
