# OpenAI 스트리밍 - 단순화 버전 (REST API 방식)
# pip install flask openai python-dotenv
#
# 2.concept 폴더의 SSE 버전과 동작은 같지만 — 포장지를 다 벗긴 형태다.
#   - 2.concept 서버: yield f"data: {json.dumps({'content': ...})}\n\n"  ← SSE 포맷으로 감싸기
#   - 여기 서버:      yield content                                       ← 토큰 텍스트 그대로
#   - 2.concept 클라: split('\n') → startsWith('data: ') → slice(6) → JSON.parse
...
if __name__ == '__main__':
    print('OpenAI 스트리밍 서버 — 단순화 (REST API 방식)')
