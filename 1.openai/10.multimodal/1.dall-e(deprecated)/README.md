...
> API에서 완전히 제거**했습니다. 예제는 '개념 학습용'으로 보존하되, 실제로 실행하려면
> `model=` 값을 현행 모델로 바꿔야 합니다.
...
이 폴더는 이미지 생성의 '개념'(생성·마스크·인페인팅)을 익히는 학습용으로 남겨두되,
실제로 동작시키려면 예제의 `model=` 값을 현행 모델(`gpt-image-1` 등)로 바꿔야 합니다.
