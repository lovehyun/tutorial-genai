# 1.vision — 비전 기초 (이미지 이해)

**Vision = 이미지를 입력으로 받아 이해하는 것** (이미지 → 텍스트).
앱(`2.vision_app*`) 이전의 **단독 기본 스크립트**다. 이미지 *생성*(텍스트 → 이미지)은 [`../../2.image_generation/4.gpt_image/`](../../2.image_generation/4.gpt_image/).
...
## 실행
```bash
cd 1.openai/9.multimodal/1.vision/1.vision
...
## 다음
- 웹앱으로 확장 → [`../2.vision_app/`](../2.vision_app/) (업로드·클립보드·음성입력 등 단계별)
