# 4.gpt_image — 이미지 생성 기초

**이미지 생성 = 텍스트를 입력으로 받아 이미지를 만드는 것** (텍스트 → 이미지).
앱(`5.gpt_image_app*`) 이전의 **단독 기본 스크립트**다. 이미지 *이해*(이미지 → 텍스트)는 [`../../1.vision/1.vision/`](../../1.vision/1.vision/).
...
## 실행
```bash
cd 1.openai/9.multimodal/2.image_generation/4.gpt_image
...
```
> 생성된 `*.png` 는 `.gitignore` 처리됨. 이미지 모델/가격 비교는 [`../5.gpt_image_app/README.md`](../5.gpt_image_app/README.md).

## 다음
- 웹앱 + 편집/인페인팅/일관성 → [`../5.gpt_image_app/`](../5.gpt_image_app/) (단계별)
