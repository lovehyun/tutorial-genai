...
| | | `1.openai/1.intro/3.sdk_new.py` | [실습] OpenAI SDK로 프롬프트 패턴 직접 실행 |
| | | `2.langchain/2.prompts/1.1_template.py` | [실습] PromptTemplate 기본 구조 |
| | | `2.langchain/2.prompts/3.1_template_chat.py` | [실습] Persona 역할 분리 (ChatPromptTemplate) |
...
| | | `2.langchain/2.prompts/2.1_template_chaining.py` | [실습] 프롬프트 체이닝 |
| | | `2.langchain/2.prompts/4.1_summarization_instruct.py` | [실습] 요약 프롬프트 패턴 |
| | | `2.langchain/2.prompts/4.7_sqlgeneration_instruct.py` | [실습] SQL 생성 프롬프트 |
| 14:45-16:00 | Context Engineering | `0.docs/05_genai_advanced/05_context_engineering.md` | 시스템 프롬프트, 토큰 예산, RAG 컨텍스트, 메모리 통합 |
| | | `2.langchain/3.structured_output/2.pydantic_parser.py` | [실습] Pydantic 파서로 구조화된 출력 |
| | | `2.langchain/3.structured_output/3.with_structured_output.py` | [실습] with_structured_output |
| 16:00-17:00 | 종합 실습 & Q&A | `2.langchain/4.chaining/2.4_runnablelambda4_chat_scoring.py` | [실습] 프롬프트 → 파서 → 후처리 전체 파이프라인, Day 1 정리 |
...
| | | `9.study/6.embedding/1.tokenize_visualize.py` | [실습] 토큰화 과정 시각화 |
| | | `9.study/6.embedding/2.embedding_visualize.py` | [실습] 임베딩 벡터 공간 시각화 |
| 10:45-12:00 | RAG 기본 구현 (Indexing & Search) | `9.study/6.embedding/4.similarity_matrix.py` | [실습] 코사인 유사도 행렬 |
| | | `1.openai/7.rag/1.faiss/1.faiss.py` | [실습] FAISS 인덱스 생성과 검색 |
| | | `2.langchain/6.RAG/2.contentloader.py` | [실습] 텍스트 로드 |
| | | `2.langchain/6.RAG/3.chromadb_saveload.py` | [실습] ChromaDB 저장/로드 |
| 13:00-14:30 | RAG 기본 구현 (Generation & 웹앱) | `2.langchain/6.RAG/1.storeandretrieve.py` | [실습] Store & Retrieve 기본 RAG |
| | | `2.langchain/6.RAG/4.pdfloader.py` | [실습] PDF 문서 로드 |
| | | `2.langchain/6.RAG/6.chromadb_search.py` | [실습] ChromaDB 유사도 검색 |
| | | `2.langchain/6.RAG/9.rag_web_app/app.py` | [실습] RAG 웹 앱 구조 확인 |
| 14:45-15:45 | 프롬프트 보안 | `0.docs/05_genai_advanced/14_evaluation_safety.md` | Jailbreak 공격 유형, Prompt Injection 패턴, 방어 전략(가드레일) |
...
| | | `2.langchain/7.agents/0.agent_cap.py` | [실습] 에이전트 기본 역량 탐색 |
| 10:45-11:15 | Single & Tool-using Agent | `2.langchain/7.agents/1.1_llmmath.py` | [실습] 수학 도구 에이전트 |
| | | `2.langchain/7.agents/2.1_wikipedia1.py` | [실습] Wikipedia 검색 에이전트 |
| | | `2.langchain/7.agents/4.1_customtools1_add.py` | [실습] @tool 데코레이터로 커스텀 도구 생성 |
| 11:15-12:00 | ReAct 패턴 | `2.langchain/7.agents/4.3_customtools1_addmultiply2_final.py` | [실습] 도구 자동 선택 ReAct 에이전트 |
| | | `2.langchain/7.agents/6.1_decide_human_serper.py` | [실습] 도구 자동 판단 라우팅 |
| | | `2.langchain/7.agents/8.1_complex.py` | [실습] 다중 도구 복합 에이전트 |
| 13:00-13:45 | Human-in-the-Loop (HITL) | `2.langchain/7.agents/5.1_humanagent.py` | [실습] 사람 개입 기본 |
| | | `2.langchain/7.agents/5.3_humanagent2_custom.py` | [실습] 커스텀 HITL |
| | | `2.langchain/7.agents/5.5_humanagent4_websearchmini.py` | [실습] HITL + 웹검색 결합 |
| 13:45-14:30 | LangGraph 기초 | `0.docs/05_genai_advanced/10_langgraph_agents.md` | StateGraph, 노드, 엣지, 조건부 분기 개념 |
| | | `2.langchain/8.langgraph/1.basic_graph.py` | [실습] 기본 StateGraph 생성 |
| 14:45-15:30 | LangGraph 실습 | `2.langchain/8.langgraph/2.memory_checkpointing.py` | [실습] 체크포인팅 |
| | | `2.langchain/8.langgraph/3.conditional_branching.py` | [실습] 조건부 분기 |
| | | `2.langchain/8.langgraph/4.tools_cyclic_graph.py` | [실습] 순환 그래프 |
| 15:30-17:00 | 에이전트 서비스 설계 | `0.docs/06_genai_applied/07_ai_agent_service.md` | 서비스 관점의 에이전트 아키텍처, 기술 스택 통합 |
| | | `2.langchain/7.agents/9.1_smartagent.py` | [실습] 스마트 에이전트 구현, Day 3 정리 |
...
| 09:30-10:00 | 패턴 1: Prompt Chaining | `2.langchain/9.agentic_patterns/1.prompt_chaining.py` | [실습] 프롬프트 순차 연결 패턴 |
| 10:00-10:30 | 패턴 2: Routing | `2.langchain/9.agentic_patterns/2.routing.py` | [실습] 입력 기반 동적 분기 패턴 |
| 10:45-11:15 | 패턴 3: Parallelization | `2.langchain/9.agentic_patterns/3.parallelization.py` | [실습] 독립 작업 병렬 실행 패턴 |
| 11:15-11:45 | 패턴 4: Orchestrator-Worker | `2.langchain/9.agentic_patterns/4.orchestrator_worker.py` | [실습] 오케스트레이터의 작업 분배 패턴 |
| 11:45-12:00 | 패턴 5: Evaluator-Optimizer | `2.langchain/9.agentic_patterns/5.evaluator_optimizer.py` | [실습] 평가 → 최적화 반복 패턴 |
| 13:00-13:45 | Tool Calling | `0.docs/05_genai_advanced/01_openai_advanced.md` | Function Calling, tool_calls, JSON Schema |
| | | `1.openai/9.structured_output/1.function_calling.py` | [실습] OpenAI Function Calling |
| | | `2.langchain/7.agents/4.4_customtools2_getinfo.py` | [실습] 정보 조회 도구 |
| 13:45-14:30 | MCP 프로토콜 연동 | `0.docs/05_genai_advanced/11_mcp_model_context_protocol.md` | MCP 아키텍처, 서버/클라이언트, 전송 계층 |
...
| 15:30-16:15 | Self-Correction Loop | `2.langchain/8.langgraph/6.self_correction_loop.py` | [실습] 출력 검증 → 자동 수정 반복 패턴 |
| | | `2.langchain/8.langgraph/5.state_debugging.py` | [실습] 그래프 상태 디버깅 |
| 16:15-17:00 | 종합 실습 | `2.langchain/8.langgraph/10.customtools1_addmultiply_langgraph.py` | [실습] 패턴 + 도구 + Self-Correction 결합, Day 4 정리 |
...
| | | `2.langchain/6.RAG/12.agentic_rag.py` | [실습] 에이전트가 검색을 자율 판단하는 Agentic RAG |
| | | `2.langchain/6.RAG/9.rag_web_app/app4_adddeletefiles.py` | [실습] 동적 문서 관리 RAG 앱 |
| 10:45-11:30 | Memory 관리 (Short-term / Long-term) | `2.langchain/5.memory/1.1_nomemory.py` | [실습] 메모리 없는 대화의 한계 체험 |
| | | `2.langchain/5.memory/3.1_memory_new.py` | [실습] Short-term: 세션 내 대화 메모리 |
| | | `2.langchain/5.memory/3.3_memory3_sqlite.py` | [실습] Long-term: SQLite 영구 메모리 |
| | | `2.langchain/5.memory/5.2_history_summary_new.py` | [실습] 대화 요약으로 컨텍스트 압축 |
| 11:30-12:00 | LangGraph 메모리 통합 | `2.langchain/5.memory/7.1_langgraph_memory.py` | [실습] LangGraph 기반 메모리 |
| | | `2.langchain/5.memory/7.2_langgraph_memory2_summary.py` | [실습] LangGraph 요약 메모리 |
| 13:00-13:45 | Safety & 보안 심화 | `0.docs/05_genai_advanced/14_evaluation_safety.md` | 가드레일 구현, 콘텐츠 필터링, LLM-as-Judge |
...
| 13:45-14:30 | Evaluation | `0.docs/05_genai_advanced/14_evaluation_safety.md` | 참조 기반/참조 없는/LLM-as-Judge 평가 프레임워크 |
| | | `0.docs/06_genai_applied/07_ai_agent_service.md` | 에이전트 서비스 평가 기준, 신뢰성 설계 |
| 14:45-15:15 | AgentOps & 모니터링 | `2.langchain/8.langgraph/7.agentops_monitoring.py` | [실습] 에이전트 실행 모니터링, 비용/지연/성공률 추적 |
...
- `0.docs/05_genai_advanced/11_mcp_model_context_protocol.md` — MCP 프로토콜
- `0.docs/05_genai_advanced/14_evaluation_safety.md` — 평가와 안전성
...
- `2.langchain/6.RAG/` — RAG 파이프라인
- `2.langchain/7.agents/` — 에이전트
- `2.langchain/8.langgraph/` — LangGraph
- `2.langchain/9.agentic_patterns/` — 5대 Agentic 패턴
- `4.anthropic/3.mcp/` — MCP 프로토콜
- `9.study/6.embedding/` — 임베딩 시각화
