...
| Level | # | 과정명 | 기간 | 시간 | 핵심 영역 |
|-------|---|--------|------|------|-----------|
| 커스텀 | 1 | [프롬프트에서 에이전트까지](4.custom/1.prompt_to_agent/) | 5일 | 40h | LLM 이해 → 프롬프트 → RAG → Agent → MCP → AgentOps |
| 커스텀 | 3 | [MCP 유연 포맷 커리큘럼](4.custom/3.mcp_flexible_formats/) | 유연 | 4h~16h | 모듈형 MCP 강의킷 — 콘솔 기초/agent tool calling 복습/여러 서버 라우팅·순차분기/HITL 등 시간 포맷별 3개 조합(4H·8H원데이·16H투데이) |

> 이론 교안(`0.docs/`)과 실습 코드를 결합한 종합 과정. 난이도별 모듈과 독립적으로 운영.
> "커스텀 3"은 고정 일정이 아니라 **모듈을 골라 조합하는 참고자료**라 위 "총 14개 과정·35일·280시간" 집계에는 포함하지 않았다.
