#!/usr/bin/env python3
# 클라↔서버 사이에 끼어 오가는 JSON-RPC 메시지를 그대로 중계하면서 debug_proxy.log 에 기록하는
...
# 클라이언트 ↔ 프록시 채널도 UTF-8 로 고정 (Windows cp949 로 JSON-RPC 가 깨지는 것 방지).
# 서버가 반환하는 한글(도구 description 등)을 print(line) 으로 재출력할 때 안전.
