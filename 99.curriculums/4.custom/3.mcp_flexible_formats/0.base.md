...
같은 교재([`8.mcp/`](../../../8.mcp/))로 **수업 길이·일수**에 맞춰 짜는 커리큘럼 모음이며,
...
| **M1** | 프로토콜 첫 접촉 | [8.mcp/1.basic/1.intro](../../../8.mcp/1.basic/1.intro/) | 30m | 첫 왕복 `initialize→list_tools→call_tool` (+[`2.protocol_deep/debug_proxy.py`](../../../8.mcp/1.basic/2.protocol_deep/debug_proxy.py)로 JSON-RPC 로그까지 보면 +10m) |
...
| **M16** | 여러 MCP 서버 라우팅 (3단 고도화) | [8.mcp/2.openai/2.multi_tools](../../../8.mcp/2.openai/2.multi_tools/) | 40m | math+utility(계산 vs 시간·날씨, 도메인이 다른 두 서버) 동시 연결 → **1~2.단독 호출**(라우팅 없음) → **3.키워드 규칙**(`find_tool`/`extract_params`) → **4.GPT function calling**(raw OpenAI SDK로 스키마→선택→실행 전 과정 노출). 실습판: [`exercise/3c~3d`](exercise/README.md). 보너스로 시간 남으면 같은 라우팅을 LangChain `MultiServerMCPClient`로 다시 보기 → [4.langchain/1.quickstart/4.multi_server.py](../../../8.mcp/4.langchain/1.quickstart/4.multi_server.py) |
