...
| 파일 | 패턴 | 설명 | 주요 기술 |
|------|------|------|----------|
| `1.prompt_chaining.py` | Prompt Chaining | 리서치→게이트→분석→보고서 파이프라인 | LCEL 체인 |
| `2.routing.py` | Routing | 고객 문의 분류 → 전문 체인 분기 | RunnableLambda |
| `3.parallelization.py` | Parallelization | 다관점 분석 + 투표 패턴 | RunnableParallel |
| `4.orchestrator_worker.py` | Orchestrator-Worker | 동적 작업 분해 및 워커 위임 | LangGraph StateGraph |
| `5.evaluator_optimizer.py` | Evaluator-Optimizer | 마케팅 카피 생성→평가→개선 루프 | LangGraph 순환 그래프 |
