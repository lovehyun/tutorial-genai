...
| `9.3_parallelization_eg1.py` | Parallelization | 팬아웃/팬인: 다관점 분석 → 종합 | RunnableParallel |
| `9.3_parallelization_eg2.py` | Parallelization | LLM-as-judge 단일 점수(0~10) → 최고 선택 | batch / 구조화 출력 |
| `9.3_parallelization_eg3.py` | Parallelization | LLM-as-judge 쌍대 비교 토너먼트(위치편향 보정) | batch / 구조화 출력 |
