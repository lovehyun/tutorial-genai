...
에이전트가 하는 일:
  ...
  → 그 자동 루프를 손으로 직접 짜보며 내부를 들여다보려면 → 4.internals/4.1_bind_tools.py
    (이름 때문에 "고급 운영 얘기"처럼 보이지만, 사실 지금 이 create_agent 가 내부적으로
     뭘 하는지 보여주는 기초 설명이다 — bind_tools 로 직접 판단→실행→재호출을 구현한다.)
