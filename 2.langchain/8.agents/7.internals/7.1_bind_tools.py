"""
bind_tools() — create_agent 가 내부적으로 쓰는 저수준 패턴.
이 예제: 도구 호출 → 결과 회수 → 최종 응답 을 직접 손으로 짜본다 (자동 루프 X).

언제 직접 이 패턴을 쓰나?
  - 도구 호출을 한 번만 받고 끝내고 싶을 때 (자동 루프 불필요)
  - create_agent 가 안에서 무슨 일을 하는지 이해하고 싶을 때
...
# ─────────────────────────────────────────────────────────────
# 이게 create_agent 가 내부에서 자동으로 돌리는 루프입니다.
# 두 번째 응답에 또 tool_calls 가 있으면 3차 호출... 끝날 때까지 반복.
# 실전에서는 직접 디스패치 대신 create_agent 를 쓰면 1~4 단계가 자동.
# ─────────────────────────────────────────────────────────────
