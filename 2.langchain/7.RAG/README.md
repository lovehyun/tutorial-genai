...
| `1.4_rag_lcel_chain.py` | retriever → LCEL 체인 (`RunnablePassthrough`) = 최초의 RAG 체인 |
...
| `2.4_chunking_pdf.py` | PDF 에 청크 전략 적용 (페이지 → 청크) |
| `2.5_metadata.py` | source / chunk_id 부착해 출처 추적 |
...
| `3.4_search_modes2_mmr.py` | MMR 심화 — 다양성·`lambda_mult` 스윕·`fetch_k`·`as_retriever(mmr)` |
...
| `3.11_metadata_filter.py` | 삽입 시 풍부한 metadata 부여 → `where` 연산자(`$in`/`$gte`/`$and`)로 필터 검색 |
...
| `4.2_with_citations.py` | 답변 끝에 출처 자동 부착 (파일 단위 unique) |
| `4.3_numbered_citations.py` | 답변에 `[1][2]` 인라인 인용 + 청크 번호별 출처 (중복 허용) |
| `4.4_builtin_chains.py` | `create_retrieval_chain` 빌트인 사용 (langchain-classic) |
...
| 파일 | 설명 / 사용 모델 |
|---|---|
| `1_instruct_basic.py` | `HuggingFacePipeline` 직접 — **평문**(chat template 미적용) 호출. base/완성형 모델용 + instruct 모델의 한계 체감 |
| `2_chat_basic.py` | 1 에서 발전 — `ChatHuggingFace` 로 **chat template 적용**. instruct/chat 모델(-instruct/-chat/-it)용 |
| `3_embeddings.py` | 영어 / 다국어 / 한국어 임베딩 비교 — all-MiniLM, bge-m3, ko-sroberta |
| `4_rag_llm_only.py` | RAG 에서 **LLM 만 HF 로 교체** (임베딩은 OpenAI 유지) — `1.ollama/2_rag.py` 와 같은 단계 |
| `5_full_rag.py` | 4 에서 **임베딩까지 HF 로** — HF LLM + HF 임베딩 = 완전 로컬 RAG |
| `6_korean_rag.py` | 5 에서 **모델만 한국어 특화로** — EXAONE-3.5 + ko-sroberta-multitask |
