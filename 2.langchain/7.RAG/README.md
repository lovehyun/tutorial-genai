...
7.local_model  ─ Ollama / HuggingFace 로 비용 없는 RAG
       ↓
8.web_app      ─ Flask 풀스택 — MVP → 출처/점수 → 파일 관리
```
...
| `1.3_first_rag.py` | retriever → prompt → llm = 최초의 RAG 체인 |
...
### `3.vectorstore/` — Chroma 영속화

| 파일 | 설명 |
|---|---|
| `3.1_persist.py` | persist_directory 저장/로드 — 재실행 시 비용 0 |
| `3.2_inspect.py` | DB 내부 살펴보기 (count / sample) |
| `3.3_search_modes.py` | similarity / with_score / MMR 비교 |
| `3.4_multi_collection.py` | 여러 컬렉션 분리 + 통합 검색 |
| `3.5_faiss_compare.py` | FAISS 비교 — 왜 Chroma 가 학습 / 일반 실무 RAG 에 더 매끄러운지 체감 |
...
### `6.agentic/` — 능동 검색 (LangGraph)

| 파일 | 설명 |
|---|---|
| `6.1_agentic_rag.py` | 검색 필요성 판단 / 쿼리 재작성 / 충분성 평가 → 적응적 루프 |
...
| `3.file_manage/` | + 파일 목록 + 삭제 (벡터까지 함께) | 5003 |
...
**Q. 검색은 잘 되는데 답이 엉뚱하다?**
→ 청크 크기 / overlap / k 값 조절. 2.3, 3.3 참고.
