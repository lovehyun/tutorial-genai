"""
QA 서비스 — answer_question 을 별도 모듈로 분리(#4 구조화 리팩토링).
이 예제: 검색→프롬프트→LLM 을 하나의 표준 LCEL 파이프라인(rag_chain)으로 묶고,
        유사도(%) + 출처 페이지를 함께 반환한다.

점수가 필요하므로 일반 retriever 대신 similarity_search_with_score 를
RunnableLambda 로 감싸 검색 단계로 사용한다(점수는 doc.metadata['score'] 에 부착).
"""

import os
from langchain_openai import ChatOpenAI
from langchain_core.prompts import ChatPromptTemplate
from langchain_core.output_parsers import StrOutputParser
from langchain_core.runnables import RunnablePassthrough, RunnableLambda

from services.vectorstore import search_with_score, is_empty

llm = ChatOpenAI(model="gpt-4o-mini", temperature=0)
prompt = ChatPromptTemplate.from_messages([
    ("system",
     "다음 문서만 참고해서 답하세요. 문서에 없으면 '모르겠습니다'.\n\n문서:\n{context}"),
    ("user", "{question}"),
])


def format_docs(docs):
    return "\n\n".join(d.page_content for d in docs)


# 검색 단계(교재 4.2 retrieve_and_split 패턴): {"question"} → docs + 포맷된 context
# 유사도 점수는 doc.metadata['score'](%) 에 부착한다.
def retrieve(inputs: dict):
    docs = []
    for doc, distance in search_with_score(inputs["question"], k=5):
        doc.metadata["score"] = round((1 - distance) * 100, 1)   # 거리 → 대략적 유사도(%)
        docs.append(doc)
    return {"question": inputs["question"], "docs": docs, "context": format_docs(docs)}


# 전체 체인: 검색 → answer 를 assign (검색 docs 도 함께 반환 → 출처/점수에 사용)
rag_chain = (
    RunnableLambda(retrieve)
    | RunnablePassthrough.assign(answer=(prompt | llm | StrOutputParser()))
)


def answer_question(question: str) -> dict:
    """답변 + 출처 리스트(파일, 페이지, 유사도) 를 dict 로 반환"""
    if is_empty():
        return {"answer": "먼저 PDF를 업로드해주세요.", "sources": []}

    # 검색+LLM 단일 파이프라인 실행 → {question, docs, context, answer}
    result = rag_chain.invoke({"question": question})
    docs = result["docs"]

    # ── 중간 결과: RAG 검색 결과를 터미널에 출력 (유사도 포함) ──
    print(f"\n>>> [RAG] 질문: {question}")
    print(f">>> [RAG] 검색된 청크 {len(docs)}개:")
    for i, doc in enumerate(docs, 1):
        src = os.path.basename(doc.metadata.get("source", "?"))
        page = doc.metadata.get("page")
        page_str = f", p.{page + 1}" if isinstance(page, int) else ""
        snippet = doc.page_content[:100].replace("\n", " ")
        print(f"    [{i}] {src}{page_str} (유사도 {doc.metadata['score']}%)\n        {snippet}...")

    # 출처 정보 정리
    sources = [{
        "file":  os.path.basename(doc.metadata.get("source", "?")),
        "page":  int(doc.metadata.get("page", 0)) + 1,
        "score": doc.metadata["score"],
    } for doc in docs]

    return {"answer": result["answer"], "sources": sources}
