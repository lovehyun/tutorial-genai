"""
Agentic RAG (3) LangGraph 고도화 — 6.2 의 똑같은 로직을 '그래프' 로 재구성.
이 예제: 검색 필요성 판단 + 쿼리 재작성 + 결과 충분성 평가를 LangGraph 로 구현.

6.2(평범한 if/for/break) → 6.3 의 차이는 '표현 방식' 뿐, 로직은 동일:
  6.2 의 함수 호출  →  6.3 의 노드(node)
  6.2 의 if/break   →  6.3 의 조건부 엣지(conditional edge)
이렇게 흐름을 그래프로 선언하면 분기/순환이 한눈에 보이고 확장도 쉬워집니다.

  ※ LangGraph 를 아직 안 배웠다면 9.langgraph 의 예고편 격입니다.
    "agentic 로직 자체" 는 6.1 / 6.2 (LangGraph 없는 버전) 로 먼저 이해하세요.
