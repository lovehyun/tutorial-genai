...
| `12.agentic_rag.py` | Agentic RAG (에이전트 기반 능동 검색) |

## Agentic RAG

기존 RAG는 항상 고정된 파이프라인(검색→응답)으로 동작하지만, Agentic RAG는 에이전트가 검색 전략을 **능동적으로 결정**합니다.

| 비교 | 기존 RAG | Agentic RAG |
|------|---------|-------------|
| 검색 여부 | 항상 검색 | 필요할 때만 검색 |
| 쿼리 | 사용자 입력 그대로 | LLM이 쿼리 재작성 |
| 검색 결과 | 그대로 사용 | 충분성 평가 후 필요 시 재검색 |
| 흐름 | 고정 (질문→검색→응답) | 적응적 루프 |

```
질문 → 검색 필요? ──No──→ 직접 응답
         │ Yes
         ↓
    쿼리 재작성 → 검색 → 충분한가? ──Yes──→ 응답 생성
                          │ No
                          └── 재작성 (최대 2회)
```
...
3. **응용**: 웹앱 → 로컬 모델 → Agentic RAG (9~12번)
