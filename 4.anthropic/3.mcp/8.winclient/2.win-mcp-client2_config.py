...
class MCPFileSystemClient:
    def __init__(self, work_directory: str = None, config_file: str = "config.json"):
        # 설정 파일 로드
        self.config = self.load_config(config_file)
        
        # 허용 디렉토리 설정
        self.allowed_directories = self.config.get("allowed_directories", [os.getcwd()])
        
        # 확장자 필터 설정
        self.allowed_extensions = self.config.get("security", {}).get("allowed_extensions", [])
        self.enable_extension_filter = self.config.get("security", {}).get("enable_file_extension_filter", False)
        
        # 작업 디렉토리 설정
        default_dir = self.config.get("default_directory", os.getcwd())
        self.work_directory = work_directory or default_dir
        self.mcp_process = None
        self.current_directory = self.work_directory
        
        # 보안: 허용된 디렉토리인지 확인
        if self.config.get("security", {}).get("enable_directory_restriction", True):
            if not self.is_directory_allowed(self.work_directory):
                raise ValueError(f"접근이 허용되지 않은 디렉토리입니다: {self.work_directory}")
    ...
    def load_config(self, config_file: str) -> Dict:
        ...
        try:
            if os.path.exists(config_file):
                ...
                print(f"⚠️ 설정 파일이 없어 기본 설정 사용: {config_file}")
    ...
    def get_default_config(self) -> Dict:
        ...
        return {
            ...
            "security": {
                "enable_directory_restriction": True,
                "enable_file_extension_filter": False,
                "allowed_extensions": [".hwp", ".docx", ".txt", ".pdf"]
            }
        }
    ...
    def is_directory_allowed(self, path: str) -> bool:
        ...
        for allowed in self.allowed_directories:
            allowed_abs = os.path.abspath(os.path.expandvars(allowed))
    ...
    def is_file_extension_allowed(self, filename: str) -> bool:
        """파일 확장자 허용 여부 확인"""
        if not self.enable_extension_filter:
            return True
