# 4.langchain/1.langchain_agent — MCP 도구를 LangChain 도구로 감싸기 (수동 변환)

MCP 도구를 LangChain `Tool`/`BaseTool` 로 **직접 감싸** ReAct 에이전트가 쓰게 한다.
`0.quickstart` 는 `langchain-mcp-adapters` 가 이 변환을 자동으로 해주는데, 여기선 손으로 해보며 원리를 본다.
...
## 실행
```bash
cd 8.mcp/4.langchain/1.langchain_agent     # 상대경로 server.py 사용 → 폴더 안에서 실행
...
## 관전 포인트
- **MCP 도구 → LangChain Tool 수동 변환** — adapters(0.quickstart)가 대신 해주던 일을 직접 → 어댑터의 가치를 체감.
