...
```python
# GPTQ 양자화 모델: Qwen/Qwen2.5-7B-Instruct-GPTQ-Int4
# AWQ 양자화 모델: hugging-quants/Meta-Llama-3.1-8B-Instruct-AWQ-INT4
...
아래 성능 유지율은 각 모델 원논문에서 보고한 벤치마크 기준(BERT 계열은 GLUE dev 평균, GPT-2 계열은 언어 모델링 perplexity 기반) 수치이며, 태스크와 평가 셋에 따라 달라질 수 있습니다.

| 모델 | 원본 (Teacher) | 파라미터 감소 | 속도 향상 | 성능 유지율 (벤치마크 기준) |
|---|---|---|---|---|
| **DistilBERT** | BERT-base (110M) | 40% 감소 (66M) | 1.6x 빠름 | GLUE ~97% |
| **TinyBERT** | BERT-base (110M) | 86% 감소 (15M) | 7.5x 빠름 | GLUE ~96% |
| **MiniLM** | BERT-base (110M) | 78% 감소 (23M) | 5.0x 빠름 | GLUE ~99% |
| **DistilGPT-2** | GPT-2 (117M) | 50% 감소 (82M) | 2.0x 빠름 | LM perplexity ~95% |
| **TinyLlama** | Llama 2 (7B) | 84% 감소 (1.1B) | 5.0x+ 빠름 | 종합 ~90% |
