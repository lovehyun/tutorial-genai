...
Claude API를 사용하려면 Anthropic 공식 Python SDK를 설치해야 합니다. `anthropic 0.69+` 버전을 기준으로 설명합니다. (기준일: 2026-06)
...
print(f"Model: {response.model}")          # claude-sonnet-4-6
...
Anthropic은 용도에 따라 여러 모델 계열을 제공합니다. 2026년 기준 주요 모델은 다음과 같습니다. (모델 인자에는 날짜 접미사 없이 alias를 사용하는 것을 권장합니다.)

| 모델 ID | 계열 | 컨텍스트 윈도우 | 최대 출력 | 특징 |
|---------|------|-----------------|-----------|------|
| `claude-opus-4-8` | Opus | 200K | 16,384 | 최고 성능, 복잡한 분석 |
| `claude-sonnet-4-6` | Sonnet | 200K | 16,384 | 성능/비용 균형, **범용 추천** |
| `claude-haiku-4-5` | Haiku | 200K | 8,192 | 빠른 속도, 저비용 |
...
|------|------------------|------------------|-----------|
| Opus 4.8 | $15.00 | $75.00 | 느림 |
| Sonnet 4.6 | $3.00 | $15.00 | 보통 |
| Haiku 4.5 | $0.80 | $4.00 | 빠름 |

### 용도별 모델 선택 가이드

| 용도 | 추천 모델 | 이유 |
|------|-----------|------|
| 프로토타입 개발 | Sonnet 4.6 | 성능과 비용의 최적 균형 |
| 단순 분류/추출 | Haiku 4.5 | 빠른 응답, 저렴한 비용 |
| 복잡한 코드 생성 | Opus 4.8 | 최고 수준의 추론 능력 |
| 고객 챗봇 | Haiku 4.5 | 실시간 응답 필요 |
| 문서 요약/분석 | Sonnet 4.6 | 긴 문서 처리 + 적절한 비용 |
| 연구/논문 분석 | Opus 4.8 | 깊은 이해와 정확한 분석 |
...
Claude Sonnet 4.6은 **확장 사고(Extended Thinking)** 기능을 지원합니다. 이 기능을 활성화하면 Claude가 답변을 생성하기 전에 **내부적으로 단계적 추론 과정**을 거칩니다. 이 사고 과정은 별도의 `thinking` 블록으로 반환되어 개발자가 확인할 수 있습니다.
...
response = client.messages.create(
    ...
    max_tokens=16000,
    thinking={
        "type": "adaptive"  # 문제 난이도에 따라 사고량을 자동 조절
    },
    messages=[
        {
            "role": "user",
...
> **핵심 포인트:** `thinking={"type": "adaptive"}`를 사용하면 Claude가 문제의 난이도를 스스로 판단하여 사고 과정에 사용할 토큰량을 **자동으로 조절**합니다. 과거에는 `budget_tokens`로 사고 토큰 상한을 직접 지정했지만, 현재는 adaptive 방식이 권장됩니다. 사고 토큰은 `max_tokens` 한도 안에서 출력 토큰과 함께 사용되므로, 복잡한 문제일수록 `max_tokens`를 충분히 크게 잡아야 합니다.
...
| 항목 | 설명 |
|------|------|
| 지원 모델 | `claude-sonnet-4-6`, `claude-opus-4-8`, `claude-haiku-4-5` |
| 사고량 제어 | `thinking={"type": "adaptive"}` — 난이도에 따라 자동 조절 |
| 토큰 한도 | 사고 토큰은 `max_tokens` 한도 내에서 출력 토큰과 함께 사용 |
...
| **확장 사고** | o-series 계열 reasoning_effort | `thinking={"type": "adaptive"}` 파라미터 |
...
5. **Extended Thinking** — `thinking={"type": "adaptive"}` 파라미터, 사고 블록 파싱
