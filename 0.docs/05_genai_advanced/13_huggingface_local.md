...
| 저장소 | 설명 | 규모 (2026년 6월 기준) |
|--------|------|------------------|
| **Models** | 사전학습된 모델 저장소 | 200만+ 모델 |
| **Datasets** | 학습/평가용 데이터셋 | 50만+ 데이터셋 |
| **Spaces** | 모델 데모 앱 (Gradio/Streamlit) | 70만+ 스페이스 |
...
```bash
# install_hf.sh -- HuggingFace 핵심 라이브러리 설치 (버전은 2026년 6월 기준 최소 권장값)
pip install "transformers>=4.52" datasets accelerate peft
...
```bash
# install_bnb.sh -- bitsandbytes 설치 (CUDA 필요, 2026년 6월 기준 최소 권장값)
...
| 양자화 방식 | 메모리 사용량 (1.5B 기준) | 추론 속도 (상대) | 품질 | 필요 환경 |
|-------------|-------------|-----------------|------|-----------|
...
### 감성분석: snunlp/KR-FinBert-SC 활용

서울대 자연어처리연구실(snunlp)에서 공개한 한국어 BERT 기반 감성분석 모델(KR-FinBert-SC)을 활용한 감성분석입니다. positive/negative/neutral 3분류 라벨을 출력합니다.
...
# 한국어 감성분석 파이프라인
# 참고: 아래 모델은 한국어 감성 코퍼스로 학습되어 positive/negative/neutral 라벨을 출력
...
| **모델 소스** | Hub (200만+ 모델) | Ollama 레지스트리 | GPT4All 모델 목록 |
