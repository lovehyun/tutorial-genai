...
| 05-09~10 | AI 에이전트 개요, LangGraph | 에이전트 실행 엔진 (StateGraph) |
...
**Supervisor 패턴**은 중앙의 **관리자 에이전트(Supervisor)**가 작업을 분석하여 적절한 **전문 에이전트(Worker)**에게 위임하는 방식입니다. 05-10강에서 학습한 LangGraph의 `StateGraph`를 활용하여 구현합니다.
...
class Router(BaseModel):
    """Supervisor의 라우팅 결정 (structured output)"""
    next: Literal["researcher", "analyst", "writer", "FINISH"]

# 상태에 라우팅 대상('next')을 담는 키를 추가
...
def supervisor_node(state: SupervisorState) -> dict:
    ...
    # structured output 으로 자유 텍스트가 아닌 정해진 라벨만 반환받음
...
def route_next(state: SupervisorState) -> Literal["researcher", "analyst", "writer", "__end__"]:
    """Supervisor가 정한 'next' 값으로 분기"""
...
# 진입 → Supervisor, Supervisor의 next 값에 따라 Worker로 분기
graph.add_edge(START, "supervisor")
graph.add_conditional_edges("supervisor", route_next)
# 각 Worker는 작업 후 다시 Supervisor로 복귀 (FINISH 까지 반복)
...
# LangSmith 환경 변수 설정 (현행: LANGSMITH_* 사용)
# 구 변수(LANGCHAIN_TRACING_V2 / LANGCHAIN_API_KEY / LANGCHAIN_PROJECT)도 하위 호환되지만 신규 변수 권장
os.environ["LANGSMITH_TRACING"] = "true"
os.environ["LANGSMITH_API_KEY"] = "lsv2_..."
os.environ["LANGSMITH_PROJECT"] = "agent-service-prod"
...
| 05-09 | AI 에이전트 개요 | 에이전트 개념, ReAct 루프 | 에이전트 설계 기초 |
| 05-10 | LangGraph | StateGraph, 멀티 에이전트 | 에이전트 실행 엔진 |
